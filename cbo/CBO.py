...
class CBO:
	...
	def __init__(self, args, data, verbose=True):
		...
		self.mean_functions_list = []
		self.var_functions_list = []
	...
	def observe(self):
		...
		self.mean_functions_list, self.var_functions_list = self.update_all_do_functions(functions)
	...
	def intervene(self):
		...
		# Compute the highest values taken by the acquisition functions, and retrieve the associated best values of xs.
		acquisition_xs, acquisition_ys = self.get_best_acquisition_values(current_best)
		...
		self.model_list[intervention].optimize()
	...
	def update_all_gaussian_processes(self):
		...
		for s in range(len(self.exploration_set)):
			model = update_gaussian_processes(
				self.mean_functions_list[s],
				self.var_functions_list[s],
	...
	def update_gaussian_process_of_last_intervention(self):
		...
		self.model_list[last_intervention] = update_gaussian_processes(
			self.mean_functions_list[last_intervention],
			self.var_functions_list[last_intervention],
			self.monitor.data_x_list[last_intervention],
			self.monitor.data_y_list[last_intervention],
			self.causal_prior
		)

	def get_best_acquisition_values(self, current_best):
		"""
		...
		# Define the lists storing the optimal values of the acquisition function, and the associated x-positions.
		xs = []
		ys = []
